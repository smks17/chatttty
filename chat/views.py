...
def chat(request):
    ...
    query = SessionModel.objects.filter(session_id=session_id, user=request.user)
    if session_id:
        session = query.first()
    else:
...
def sessions(request):
    sessions = SessionModel.objects.filter(user=request.user).values("session_id", "session_name", "created_date").order_by("-created_date")
